...
export const signIn = async (email: string, password: string): Promise<User> => {
    // If Supabase isn't configured, default straight to local storage without attempting network request
    if (!isSupabaseConfigured) {
        ...
        throw new Error("User not found (Offline Mode). Please Sign Up to create a local account.");
    }
    ...
    try {
        ...
        if (isNetworkError(error)) {
            ...
            // If offline and user not found locally, throw a clear error
            throw new Error("Connection failed (Offline Mode). User not found on this device. Please Sign Up to create a local account.");
        }
        throw error;
    }
};
...
export const signUp = async (email: string, password: string, username: string, firstName: string, lastName: string, plan: any, cycle: any, isAdmin: boolean = false): Promise<User> => {
    // Helper to create a local user object
...
export const signInWithGoogle = async (): Promise<User> => {
    if (!isSupabaseConfigured) {
        throw new Error("Google Sign-In is unavailable in Offline Mode. Please use email/password.");
    }
...
export const changePassword = async (current: string, newPass: string): Promise<void> => {
    if (!isSupabaseConfigured) {
        // In offline mode, we can't truly update auth password securely without re-hashing logic 
        // that isn't implemented here. Just resolve for demo purposes.
